...
### Phase 5 findings — descriptions and enrichment

**The free path covered everything.** Of 218 canonical articles, 146 descriptions came from the RSS feed and 72 from the publisher's own `og:description`. **Zero were generated by a model** — the fallback exists but never fired, which is the intended outcome: publishers already write these for social previews.

**Measured enrichment quality** (`scripts/eval_enrich.py`, 46 hand-labelled articles):
...
Adding a one-line description per category fixed executive appointments and legal settlements, both of which had been defaulting to `Other`. The fixture allows defensible alternates per article, because scoring "is a stake sale M&A or Market/Analysis?" as a hard error measures the labeller's opinion, not the model.

**Actual cost:** 218 articles enriched in 8 batches for **$0.0105** — $0.000048 each, matching the estimate.

**Bug worth recording:** the first enrichment run died with `database is locked`. The loop held one write transaction open across every model call, while `record_call` needs its own connection for the ledger. Reads, the model call, and writes are now three separate steps — holding a write lock across a multi-second network call is wrong regardless of the database. A `busy_timeout` pragma was added as defence in depth.


### Phase 6 findings — API and frontend

**Design brief.** The audience tracks Indian deal flow, so the job is scan-and-triage, not reading. Two choices follow from that and drive the whole layout:
- **Company reads before headline.** This audience scans for *who*, which inverts the usual news hierarchy.
- **Category is a coloured spine, not a pill** — it is a filter dimension, so it earns structural encoding rather than decoration.

The organising device is a time-anchored tape with day rules, not cards and not newspaper columns. One family for interface and data (Instrument Sans, tabular figures), one for the journalism itself (Newsreader) — a semantic split, not ornament.

**Fixed after looking at a screenshot**, which is the only way these showed up:
- Descriptions ran to seven lines, defeating a triage feed. Clamped to two.
- The activity bar was meant to be the one bold element and was the quietest thing on the page. Given a baseline and solid fill so it reads as a chart.

**A third dedupe gap, visible only in the rendered feed:** Ultraviolette's $85M round appeared twice. The two headlines shared just the company and the amount, scoring 0.40 on headline similarity. Enrichment now supplies the company, so `dedupe_by_company()` runs as a **second pass** after it — same company, same category, no conflicting figures, inside 48h, and not a roundup.

Two refinements the real data forced:
- **Calendar years are not deal figures.** `{85, 2027}` against `{85}` failed strict equality and kept one round split in two. `significant_figures()` drops years; a company's Series F and Series G still differ on the amount.
- **Company plus category is too weak without figures.** It merged two separate remarks by one executive at one event, and merged a specific story into a multi-topic roundup. Now a figure-free match also needs headline similarity above a floor, and the multi-topic guard applies.

Duplicates caught rose from 18 to **32 of 236**. Remaining known gap: a deal described by *valuation* in one outlet and *round size* in another ("Brahma AI at $2 bn valuation" vs "raises $150M") reads as conflicting figures and stays unmerged. That errs toward showing a duplicate rather than hiding a story, which is the right direction.


### Phase 7 findings — the sidebar

**The natural-language filter never sees an article.** It gets the *schema* and the reader's phrase, returns a JSON filter, and that runs as SQL. Measured at **$0.00009 per phrase**, half the estimate, and it stays exact at 10,000 articles because cost does not scale with the corpus. Identical phrases are cached and return at $0.000000.

Verified against real phrases:
...
The last one matters: **fintech became a search term, not an invented category.** Everything the model returns is treated as untrusted — unknown categories and sources are dropped rather than passed to SQL.

**Explain and summarise read only what is already stored** — headline, publisher description, and the one-line summary written during enrichment. Nothing re-fetches an article or re-summarises text already paid for. Explain costs ~$0.0001; summarising 30 stories ~$0.0005, returned grouped with citations back to story numbers.

**Sonnet 4.6 is an explicit opt-in checkbox** labelled with its real cost, never a default.

**Bug worth recording:** the first end-to-end run returned `405 Method Not Allowed` on every POST. The server was still the process started in Phase 6, before these routes existed — the SPA catch-all `GET /{full_path}` matched the path but not the method. Restarting fixed it; the lesson is that a long-lived dev server silently serves stale routes.
